    ...
    <div className="bg-surface text-on-surface font-body-md min-h-screen flex flex-col">
      ...
      <header className="bg-surface-container-lowest border-b border-outline-variant fixed top-0 w-full z-50 flex justify-between items-center px-gutter h-16 max-w-container-max mx-auto">
        ...
        <div className="flex items-center gap-lg">
          ...
          <nav className="hidden md:flex items-center gap-md">
            ...
            <Link to={'/upload'} className="text-on-surface-variant font-medium hover:text-primary transition-colors duration-200">Upload</Link>
            <Link to={'/certificates'} className="text-on-surface-variant font-medium hover:text-primary transition-colors duration-200">Certificates</Link>
          </nav>
          <div className="flex items-center gap-sm">
            <Link to={'/upload'} className="bg-primary-container text-on-primary-container px-4 py-2 rounded-lg font-label-md hover:bg-primary-fixed-variant transition-colors shadow-sm focus:ring-2 focus:ring-primary/20">
              Get Started
            </Link>
          </div>
        </div>
        ...
        <section className="max-w-container-max mx-auto px-gutter py-xl md:py-24 grid grid-cols-1 md:grid-cols-2 gap-xl items-center">
          <div className="space-y-lg">
            ...
            <p className="font-body-lg text-body-lg text-on-surface-variant max-w-2xl">
              Prevent fake certificates using secure blockchain technology. Issue, verify, and store credentials with absolute cryptographic certainty.
            </p>
            <div className="flex flex-wrap gap-sm">
              <Link to={'/upload'} className="bg-primary-container text-on-primary px-6 py-3 rounded-lg font-label-md text-label-md hover:opacity-90 transition-opacity shadow-md hover:shadow-lg focus:ring-2 focus:ring-primary/20 flex items-center gap-2">
                <span className="material-symbols-outlined" data-icon="upload_file">upload_file</span>
                Upload Certificate
              </Link>
              <Link to={'/certificates'} className="bg-surface-container-lowest text-on-surface border border-outline-variant px-6 py-3 rounded-lg font-label-md text-label-md hover:border-primary hover:text-primary transition-colors shadow-sm hover:shadow-md focus:ring-2 focus:ring-primary/20 flex items-center gap-2">
        ...
        <section className="bg-surface-container-low py-xl">
          <div className="max-w-container-max mx-auto px-gutter">
            <div className="text-center mb-xl">
              <h2 className="font-headline-lg text-headline-lg text-on-surface mb-sm">Uncompromising Security &amp; Speed</h2>
              <p className="font-body-md text-body-md text-on-surface-variant max-w-2xl mx-auto">
                Built on advanced blockchain infrastructure to ensure your credentials are safe, instantly verifiable, and immune to tampering.
              </p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-md">
              ...
              <div className="bg-surface-container-lowest p-lg rounded-xl border border-outline-variant hover:border-primary hover:shadow-lg transition-all duration-300 group">
                <div className="w-12 h-12 bg-primary-container/10 rounded-lg flex items-center justify-center mb-md group-hover:bg-primary-container/20 transition-colors">
                  <span className="material-symbols-outlined text-primary-container" data-icon="lock">lock</span>
                </div>
                <h3 className="font-headline-md text-headline-md text-on-surface mb-sm">Secure Blockchain Storage</h3>
                <p className="font-body-md text-body-md text-on-surface-variant">
                  Store immutable certificate records securely on a decentralized network, eliminating single points of failure.
                </p>
              </div>
              {/* Feature 2 */}
              <div className="bg-surface-container-lowest p-lg rounded-xl border border-outline-variant hover:border-primary hover:shadow-lg transition-all duration-300 group md:col-span-1">
                ...
                <h3 className="font-headline-md text-headline-md text-on-surface mb-sm">Instant Verification</h3>
                <p className="font-body-md text-body-md text-on-surface-variant">
                  Verify certificate authenticity in seconds with our lightning-fast validation protocol, without relying on manual checks.
                </p>
              </div>
              {/* Feature 3 */}
              <div className="bg-surface-container-lowest p-lg rounded-xl border border-outline-variant hover:border-primary hover:shadow-lg transition-all duration-300 group md:col-span-1">
                ...
                <h3 className="font-headline-md text-headline-md text-on-surface mb-sm">Tamper Proof</h3>
                <p className="font-body-md text-body-md text-on-surface-variant">
                  Cryptographic hashing guarantees that certificates cannot be modified or forged once registered on the chain.
                </p>
              </div>
            </div>
          </div>
      ...
      <footer className="bg-surface-container w-full py-lg px-gutter flex flex-col md:flex-row justify-between items-center gap-md border-t border-outline-variant">
        ...
        <nav className="flex flex-wrap justify-center gap-md font-label-sm text-label-sm">
          <a className="text-on-surface-variant hover:text-primary transition-colors focus:ring-2 focus:ring-primary/20 rounded" href="#">About</a>
          <a className="text-on-surface-variant hover:text-primary transition-colors focus:ring-2 focus:ring-primary/20 rounded" href="#">Contact</a>
          <a className="text-on-surface-variant hover:text-primary transition-colors focus:ring-2 focus:ring-primary/20 rounded" href="#">Documentation</a>
          <a className="text-on-surface-variant hover:text-primary transition-colors focus:ring-2 focus:ring-primary/20 rounded" href="#">Privacy Policy</a>
        </nav>
        <div className="font-body-md text-body-md text-on-surface-variant">© 2026 CertChain. All rights reserved.</div>
      </footer>
    </div>
  );
};
